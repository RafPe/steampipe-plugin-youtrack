...
Returns agile boards visible to the permanent-token user. Missing optional YouTrack values are SQL null; nested or polymorphic values remain JSONB and `raw` preserves the requested representation.
...
```sql
select * from youtrack_agile where id = 'example-id';
```

```sql
select id, name from youtrack_agile limit 20;
```
