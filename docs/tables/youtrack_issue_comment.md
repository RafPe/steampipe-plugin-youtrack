...
Returns issue comments visible to the permanent-token user. Missing optional YouTrack values are SQL null; nested or polymorphic values remain JSONB and `raw` preserves the requested representation.
...
```sql
select * from youtrack_issue_comment where id = 'example-id';
```

```sql
select id, name from youtrack_issue_comment limit 20;
```
