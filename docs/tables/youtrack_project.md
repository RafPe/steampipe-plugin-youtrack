...
Returns projects visible to the permanent-token user. Missing optional YouTrack values are SQL null; nested or polymorphic values remain JSONB and `raw` preserves the requested representation.
...
```sql
select * from youtrack_project where id = 'example-id';
```

```sql
select id, name from youtrack_project limit 20;
```
