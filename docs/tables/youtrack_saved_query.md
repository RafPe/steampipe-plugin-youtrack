...
Returns saved queries visible to the permanent-token user. Missing optional YouTrack values are SQL null; nested or polymorphic values remain JSONB and `raw` preserves the requested representation.
...
```sql
select * from youtrack_saved_query where id = 'example-id';
```

```sql
select id, name from youtrack_saved_query limit 20;
```
